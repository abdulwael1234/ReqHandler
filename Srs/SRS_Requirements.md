...
| SRS-019 | C | The following work-specific items are intentionally deferred until the complete repository is transferred to the work computer: (a) supported input formats, (b) source identifiers and their mapping to `source_reference`, (c) exact R210 output templates and file format, (d) file and artifact naming conventions, (e) source-input adapters, (f) AUTOSAR package and output paths, (g) metamodel/version identifiers and access-point mapping rules, and (h) port-interface compatibility rules. Owner: development team on the work computer. Closure condition: each item is documented with concrete values and validated against real work data without copying real work documents, identifiers, paths, templates, or configuration into the external-development environment. Until closure, synthetic placeholders shall be used and work-specific generation shall remain disabled. | §2, §9 |
...
| SRS-036a | D | The cross-artifact type-reference foreign keys (`element_type_id` in `ArrayTypeDefinitions` and `StructElements`; `type_definition_id` in `InterfaceDataElements` and `OperationArguments`) may be stored as `NULL` while unresolved. Creation with an unresolved reference shall create a `ReviewIssue` with `issue_type = 'unresolved_reference'`. A record with an unresolved type reference shall not be approved or exported. The reference shall be resolved through a later update before approval. | §5.1 (derived, approved 2026-08-12) |
...
| SRS-072 | F | Connection validation shall verify that the connection contains at least one provider and at least one requester, determined from the `direction` field of the referenced port prototypes. | §5.6 (approved 2026-08-12) |
...
| SRS-036a | Allow `NULL` while the four cross-artifact type references are unresolved; create an `unresolved_reference` issue and block approval/export until resolved | Supports forward and circular references without inventing targets. | **Approved 2026-08-12** |
| SRS-046, SRS-053, SRS-092a | Parent status blocked from `approved` when any non-rejected child is not `approved`; rejected children excluded from approval and export evaluation; reviewer picks the specific parent status | Prevents rejected children from permanently blocking a parent when deletion is prohibited while retaining manual review as the quality gate. | **Approved 2026-08-12** |
| SRS-072 | At least one provider and one requester per connection | Provides the minimum meaningful direction cardinality while still allowing multi-provider and multi-requester connections. | **Approved 2026-08-12** |
...
| 5.4     | 2026-08-12 | Approved the remaining technical decisions: nullable unresolved type references with issue creation and approval/export blocking (SRS-036a), rejected-child exclusion and non-rejected-child approval gating (SRS-046/053/092a), and minimum connection cardinality of one provider plus one requester (SRS-072). Amended SRS-019 to defer all work-specific formats, identifiers, templates, naming, adapters, package paths, and configuration until repository transfer to the work computer, with synthetic placeholders only outside that environment. |
